...
export default function TodayDaySheet({
  ...
  // prettier-ignore
  return <section className="today-sheet" aria-labelledby="today-sheet-title"><header className="today-sheet__header"><p>Execution</p><h2 id="today-sheet-title">Day Sheet</h2><span>{blockCount} planned {blockCount === 1 ? 'block' : 'blocks'}</span></header>{statusMessage ? <p className="today-sheet__status" role="status" aria-live="polite">{statusMessage}</p> : null}{blockCount === 0 ? <div className="today-sheet__available"><h3>No planned blocks</h3><p>The day is available from 00:00 to 24:00.</p><a href="/app/planning">Open Planning</a></div> : null}<ol className="today-sheet__list" aria-label="Chronological day sheet">{entries.map((entry) => <li key={entry.key} className={`today-sheet__item today-sheet__item--${entry.kind}`}>{entry.kind === 'current' ? <div className="today-sheet__now" aria-current="time">Now · {formatTimeLabel(entry.at)}</div> : entry.row.kind === 'gap' ? <GapRow row={entry.row} currentTime={entry.currentTime} /> : <TodayBlockRow row={entry.row} detail={blockDetails[entry.row.block.id] ?? null} currentTime={entry.currentTime} actionPath={actionPath} />}</li>)}</ol><style>{styles}</style></section>;
}
...
function GapRow({
  ...
  const endLabel = isFullDayGap(row) ? '24:00' : formatTimeLabel(row.end);
  // prettier-ignore
  return <div className="today-sheet__gap"><span>Open time</span><span className="today-sheet__gap-times"><time dateTime={row.start}>{formatTimeLabel(row.start)}</time>{currentTime ? <> · <span className="today-sheet__gap-current" aria-current="time">Now · {formatTimeLabel(currentTime)}</span> · </> : ' · '}<time dateTime={row.end}>{endLabel}</time></span></div>;
}

function isFullDayGap(row: Extract<DaySheetRow, { kind: 'gap' }>) {
  const duration = Date.parse(row.end) - Date.parse(row.start);
  return (
    duration === 24 * 60 * 60 * 1000 &&
    formatTimeLabel(row.start) === '00:00' &&
    formatTimeLabel(row.end) === '00:00'
  );
}
...
.today-sheet__gap,.today-sheet__now,.today-sheet__available{border:1px dashed var(--chronos-border,var(--border,#cbd5e1));border-radius:.75rem;padding:.75rem;background:var(--chronos-surface-muted,var(--muted,#f1f5f9));display:flex;justify-content:space-between;gap:.75rem;flex-wrap:wrap}.today-sheet__gap-times{display:flex;align-items:baseline;gap:.25rem;flex-wrap:wrap;max-width:100%;font-variant-numeric:tabular-nums}.today-sheet__gap-current{color:var(--chronos-primary,var(--primary,#4f46e5));font-weight:800}.today-sheet__now{color:var(--chronos-primary,var(--primary,#4f46e5));font-weight:800}.today-sheet__available{display:block;margin-top:1rem}.today-sheet__available h3,.today-sheet__available p{margin:0 0:.5rem}
